...
def get_secret_details(secret_name, namespace="default", access_level=AccessLevel.VIEWER):
    try:
        utils._load_k8s_config()
        api = client.CoreV1Api()
        ...
        embed = {
            "title": f"Secret Details: {secret.metadata.name}",
            "color": 3447003,
            "fields": [
...
def patch_secret_data(secret_name, key, value, namespace="default", access_level=AccessLevel.ADMIN):
    try:
        utils._load_k8s_config()
        api = client.CoreV1Api()
        ...
        return get_secret_details(secret_name, namespace, access_level)
    except client.ApiException as e:
        return (4, {"content": f"❌ Error while updating `{secret_name}`: `{e.reason}`", "flags": 64})

def delete_secret_key(secret_name, key, namespace="default", access_level=AccessLevel.ADMIN):
    try:
        utils._load_k8s_config()
        api = client.CoreV1Api()
